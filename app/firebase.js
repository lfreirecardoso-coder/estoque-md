...
const firebaseConfig = {
  ...
  // <<< MUITO IMPORTANTE: bucket correto (.firebasestorage.app)
...
(function init() {
  ...
  last.onload = () => {
    try {
      ...
      // Usa explicitamente o bucket que aparece no Console:
      // gs://estoque--md.firebasestorage.app
      const storage = firebase.app().storage("gs://estoque--md.firebasestorage.app");

      // Exporta serviços globais para o app
      window.firebase = firebase;
      window.auth     = firebase.auth();
      window.db       = firebase.firestore();
      window.storage  = storage;

      // Dispara evento para o app.js iniciar
